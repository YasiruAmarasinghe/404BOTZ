let handler = async (m, { conn, command, text }) => {
  let type = command.replace(/^set(menu|help|\?|panel)/, '').toLowerCase()
  if (type == '') {
    if (text) {
      conn.menu = text
      conn.reply(m.chat, 'menu success set\n' + info, m)
    } else {
      conn.menu = {}
      conn.reply(m.chat, 'Menu reseted', m)
    }
  } else {
    conn.menu = typeof conn.menu == 'object' ? conn.menu : {}
    if (text) {
      conn.menu[type] = text
      conn.reply(m.chat, 'Menu ' + type + ' success set\n' + info, m)
    } else {
...
handler.tags = ['owner']
handler.command = /^set(menu|help|\?|panel)(before|header|body|footer|after)?$/i
handler.owner = true
handler.mods = false
