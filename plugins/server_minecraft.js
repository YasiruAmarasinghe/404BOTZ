...
`.trim())
                break
            default:
            return m.reply(`Wrong format ${usedPrefix}server <bedrock | java> <ip> <port>\nExample usage: *${usedPrefix}server bedrock play.nethergames.org 19132*`.trim())
        }
    } catch (e) {
        conn.reply(m.chat, `Wrong format ${usedPrefix}server <bedrock | java> <ip> <port>\nExample usage: *${usedPrefix}server bedrock play.nethergames.org 19132*`.trim(), m)
        console.log(e)
        if (DevMode) {
