...
let badwordRegex = /anj(k|g)|ajn?(g|k)|a?njin(g|k)|babi|bajingan|b(a?n)?gsa?t|ko?nto?l|me?me?(k|q)|pe?pe?(k|q)|meki|titi(t|d)|pe?ler|tetek|toket|ngewe|go?blo?k|to?lo?l|idiot|(k|ng)e?nto?(t|d)|jembut|bego|dajj?al|janc(u|o)k|pantek|puki ?(mak)?|kimak|kampang|lonte|col(i|mek?)|pelacur|henceu?t|nigga|fuck|dick|bitch|tits|bastard|asshole/i // tambahin sendiri
...
handler.before = function (m, { isOwner, isBotAdmin }) {
    ...
    if (!chat.antiBadword && !chat.isBanned && isBadword) {
        user.warning += 1
        this.sendButton(m.chat, `*Badword Detected!*
...
“Never use a big word when a little filthy one will do” (404bot).`, watermark, 'Turn on antibadword', ',sorry', m)
        if (user.warning >= 5) {
            user.banned = true
            if (m.isGroup) {
                if (isBotAdmin) {
