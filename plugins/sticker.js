...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  try {
    ...
    if (/webp/.test(mime)) {
      ...
      if (!img) throw ` reply to stiker ${usedPrefix + command}`
      ...
      let img = await q.download()
      if (!img) throw `Reply to image ${usedPrefix + command}`
      wsf = new WSF.Sticker(img, {
        pack: global.packname,
        author: global.author,
        crop: false,
      ...
      if ((q.msg || q).seconds > 11) throw 'Max 10 seconds!'
      let img = await q.download()
      if (!img) throw `Reply to video ${usedPrefix + command}`
      wsf = new WSF.Sticker(img, {
        pack: global.packname,
        author: global.author,
