...
let handler = async (m, { conn, usedPrefix: _p }) => {
  ...
  ['Info',  '/info'],
  ['Owner',  '/owner']
], { quoted: m}).catch(_ => conn.send3buttonImg(m.chat, pp, 'menu.jpg', text.trim(), 'PING', '.ping', 'RULES', '.rules', 'STATUS', '.botstatus', m)).catch(_ => conn.reply(m.chat, text.trim(), 'PING', '.ping', 'RULES', '.rules', 'STATUS', '.botstatus', m))
  } catch (e) {
...
handler.help = ['test', 'boted']
handler.tags = ['main']
handler.command = /^(test|boted)$/i
handler.owner = false
handler.mods = false
